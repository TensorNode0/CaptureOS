...
export default function VentureWorkspace({ title, sectionLabel, blurb, kinds, testid }) {
  ...
  return (
    <PageReveal className="space-y-5">
      ...
      <AIChatButton
        contextTitle={title}
        contextText={(docs || []).slice(0, 15).map((d) =>
          `--- ${d.title} (${d.kind}) ---\n${(d.contentMd || "").slice(0, 3500)}`).join("\n\n")}
        suggestions={[
          "Suggest 5 improvements to the top draft.",
          "Rewrite the executive summary for a defense investor.",
          "What's missing in this pitch that VCs will ask?",
        ]}
      />
    </PageReveal>
  );
...
function EditModal({ doc, onClose, orgId, onSaved }) {
  ...
  if (!doc) return null;
  // Accelerator applications generated by the AI carry a structured schema in
  // content_json — render it as a fillable form instead of a markdown textarea.
  const isAcceleratorForm =
    doc.kind === "accelerator_application" &&
    doc.contentJson?.kind === "acceleratorApplication" &&
    Array.isArray(doc.contentJson.questions);
  const isJson = !isAcceleratorForm && !doc.contentMd
                 && doc.contentJson && Object.keys(doc.contentJson).length;
  const save = async () => {
    setSaving(true);
    try {
  ...
  const redraftForm = async () => {
    setRedrafting(true);
    try {
      await api.post(`/orgs/${orgId}/venture-docs/${doc.id}/redraft-form`);
      toast.success("Form redrafted with AI");
      onSaved();
    } catch (e) { toast.error(errMsg(e)); }
